...
for ($i=0, $n=count($attachments); $i < $n; $i++) {
	...
	if ( $this->show_file_links ) {
		...
		$html .= "<a class=\"at_icon\" href=\"$url\"$target title=\"$tooltip\" onClick=\"_gaq.push(['_trackEvent','News','Download','$filename']);\"><img src=\"$icon_url\" alt=\"$tooltip\" />";
		...
		$html .= "</a>";
		$html .= "<a class=\"at_url\" href=\"$url\"$target target=\"_blank\" title=\"$tooltip\" onClick=\"_gaq.push(['_trackEvent','News','Download','$filename']);\">$filename</a>";
		}
	else {
		$tooltip = JText::sprintf('ATTACH_DOWNLOAD_THIS_FILE_S', $actual_filename);
