...
<div class="blog-item-cat">
	<?php if ($params->get('show_parent_category') && $this->item->parent_id != 1) : ?>
		<span class="parent-category-names">
...
<footer class="article-info">
    <header style="display:none"><h3>Beitragsinformationen: <?php echo $this->escape($this->item->title); ?></h3></header>
        ...
        <?php if ($params->get('show_publish_date') && $params->get('show_author') && !empty($this->item->author )) : ?>
            <p class="createdby published">
                <?php $author =  $this->item->author; ?>
                <?php $author = ($this->item->created_by_alias ? $this->item->created_by_alias : $author);?>
                
    			<?php if (!empty($this->item->contactid ) &&  $params->get('link_author') == true):?>
    				<?php 	echo JText::sprintf('COM_CONTENT_WRITTEN_BY_SHORT' ,
    				 JHtml::_('link', JRoute::_('index.php?option=com_contact&view=contact&id='.$this->item->contactid), $author)); ?>
    
    			<?php else :?>
    				<?php echo JText::sprintf('COM_CONTENT_WRITTEN_BY_SHORT', $author); ?>
    			<?php endif; ?>
                <?php echo JText::sprintf('COM_CONTENT_AND_PUBLISHED_DATE_ON_2', JHtml::_('date', $this->item->publish_up, JText::_('DATE_FORMAT_LC3'))); ?>
            </p>
        <?php endif; ?>
...
<?php if (($params->get('show_create_date')) or ($params->get('show_modify_date')) or ($params->get('show_hits'))) : ?>
...
<?php endif; ?>
<?php if (($params->get('show_create_date')) or ($params->get('show_modify_date'))or ($params->get('show_parent_category')) or ($params->get('show_hits'))) : ?>
 	</dl>
<?php endif; ?>
</article>
<?php if ($this->item->state == 0) : ?>
